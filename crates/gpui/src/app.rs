...
impl App {
    ...
    ///
    /// Unless overridden via [`Self::set_reduce_motion`], this follows the
    /// operating system's reduced motion accessibility preference.
    pub fn reduce_motion(&self) -> bool {
        self.reduce_motion
            .unwrap_or_else(|| self.platform.should_reduce_motion())
    }

    /// Sets whether non-essential animations (e.g. loading spinners) should be
    /// rendered in a static state instead of animating.
    ///
    /// Passing `None` follows the operating system's reduced motion
    /// accessibility preference.
    pub fn set_reduce_motion(&mut self, reduce_motion: Option<bool>) {
