...
fn init_reduce_motion(cx: &mut App) {
    let apply = |cx: &mut App| {
        let reduce_motion = match ReduceMotionSetting::get_global(cx).0 {
            settings::ReduceMotionMode::On => Some(true),
            settings::ReduceMotionMode::Off => Some(false),
            settings::ReduceMotionMode::System => None,
        };
